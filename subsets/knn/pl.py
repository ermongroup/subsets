...
class PL(Distribution):
    ...
    def relaxed_sort(self, inp):
        ...
        if self.hard:
            ...
            b_idx = torch.arange(bsize).repeat([1,dim]).view(dim,bsize).transpose(dim0=1, dim1=0).flatten().type(LongTensor)
    ...
    def rsample(self, sample_shape, log_score=True):
        ...
        with torch.enable_grad(): # torch.distributions turns off autograd
          ...
          def sample_gumbel(samples_shape, eps=1e-20):
              # U = torch.rand(samples_shape)  # this needs to be cuda!
              # U = FloatTensor(samples_shape).uniform_()
          ...
          log_s_perturb = log_s_perturb.view(-1, self.n, 1) # -1 = n_samples * batch_size
...
if __name__ == '__main__':
    ...
    bad_pm = torch.Tensor([[[0., 1., 0.],
         [1., 0., 0.],
         [0., 0., 1.]]])
    print(pl_dist.log_prob(good_pm), pl_dist.log_prob(intermediate_pm), pl_dist.log_prob(bad_pm))
    print()
    ...
    print(pl_dist_prior.log_prob(good_pm), pl_dist_prior.log_prob(intermediate_pm), pl_dist_prior.log_prob(bad_pm))
